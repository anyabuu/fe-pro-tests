...
describe('objectEntries()', () => {
    it('It remade object in array', () => {
        const result = objectEntries({first: 'first', second: 'second', third: 'third'});

        expect(result).toEqual([['first', 'first'], ['second', 'second'], ['third', 'third']]);
    });
});
...
describe('addDotsToString()', () => {
    describe('If permissed length is less than string length', () => {
        it('It returns sliced string', () => {
            const result = addDotsToString('Слишком мало места для всего, что тут можно написать', 6);
    ...
    describe('If permissed length is more than string length', () => {
        it('It returns all string', () => {
            const result = addDotsToString('Слишком мало места для всего, что тут можно написать', 888);

            expect(result).toEqual('Слишком мало места для всего, что тут можно написать');
        });
    });
});
